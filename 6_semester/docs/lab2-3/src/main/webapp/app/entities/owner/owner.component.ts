...
export class OwnerComponent implements OnInit, OnDestroy {
  ...
  loadAll(): void {
    this.ownerService
      ...
      .subscribe((res: HttpResponse<IOwner[]>) => this.paginateOwners(res.body, res.headers));
  }
  ...
  registerChangeInOwners(): void {
    this.eventSubscriber = this.eventManager.subscribe('ownerListModification', () => this.reset());
  }
  ...
  protected paginateOwners(data: IOwner[] | null, headers: HttpHeaders): void {
    const headersLink = headers.get('link');
    this.links = this.parseLinks.parse(headersLink ? headersLink : '');
    if (data) {
      for (let i = 0; i < data.length; i++) {
        this.owners.push(data[i]);
      }
    }
  }
}
