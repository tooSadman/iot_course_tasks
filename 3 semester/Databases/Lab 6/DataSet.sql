...
CREATE PROCEDURE insert_hangaraircraft(
  ...
  IN aircraft_name_in VARCHAR(30)
)

  BEGIN
    DECLARE msg VARCHAR(50);
    -- checks if player exists
    IF NOT EXISTS(SELECT * FROM aircraft WHERE pilot = pilot_in AND aircraft_name = aircraft_name_in)
    THEN SET msg = ''There is no such aircraft'';
    -- checks if hangar city exists
    ELSEIF NOT EXISTS(SELECT * FROM hangar WHERE hangar_city = hangar_city_in)
      THEN SET msg = ''There is no such battle'';
    -- checks if such combinations exists
    ELSEIF EXISTS(
        SELECT * FROM hangaraircraft WHERE
            pilot = (SELECT pilot FROM aircraft WHERE pilot = pilot_in AND aircraft_name = aircraft_name_in) AND
            hangar_city = (SELECT hangar_city FROM hangar WHERE hangar_city = battle_name_in)
    ) THEN SET msg = ''This aircraft already was in this battle'';
...
CREATE PROCEDURE update_hangar(
  IN old_hangar_city_in VARCHAR(50),
  IN new_hangar_city_in VARCHAR(50)
)
  ...
  BEGIN
    ...
    IF EXISTS (SELECT * FROM hangar WHERE hangar_city = old_hangar_city_in)
    THEN UPDATE hangar SET hangar_city = new_hangar_city_in
         WHERE hangar_city = old_hangar_city_in; SET msg = ''OK'';

    ELSE SET msg = ''No such battle'';
    END IF;
  END //
